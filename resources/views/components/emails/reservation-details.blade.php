...
@if ($reservation->guests->isNotEmpty())
    <div style="font-family:Georgia, 'Times New Roman', serif; color:#2f2923; font-size:18px; font-weight:bold; margin:0 0 10px;">Your guests</div>
    <div style="color:#584c40; font-size:14px; line-height:1.75; margin-bottom:26px;">
        @foreach ($reservation->guests as $guest)
            <div>{{ $guest->full_name }} <span style="color:#9a7954;">· {{ $guest->guest_type === 'adult' ? 'Adult' : 'Child' }}</span></div>
        @endforeach
    </div>
@endif
